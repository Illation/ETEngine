...
public:
	...
	matrix();
	explicit matrix(math::ctor) {} //uninitialized constructor
	matrix(const std::initializer_list<T> args);
	matrix(const math::vector<n, T> rowList[m]);
...
//shorthands
typedef math::matrix<2, 2, float>  mat2;
typedef math::matrix<3, 3, float>  mat3;
typedef math::matrix<4, 4, float>  mat4;
typedef math::matrix<2, 2, double> dmat2;
typedef math::matrix<3, 3, double> dmat3;
typedef math::matrix<4, 4, double> dmat4;
