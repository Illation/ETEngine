...
public:
	...
	std::vector<rhi::UniformParam> const& GetUniformLayout() const { return m_UniformLayout; }
	std::vector<core::HashString> const& GetUniformIds() const { return m_UniformIds; }
	rhi::T_ConstParameterBlock GetCurrentUniforms() const { return m_CurrentUniforms; }

	// functionliaty
	//---------------------
	rhi::T_ParameterBlock CopyParameterBlock(rhi::T_ConstParameterBlock const source) const;
	void UploadParameterBlock(rhi::T_ConstParameterBlock const block) const;
...
private:
	...
	// loose uniforms
	std::vector<rhi::UniformParam> m_UniformLayout;
	std::vector<core::HashString> m_UniformIds;
	rhi::T_ParameterBlock m_CurrentUniforms = nullptr;
