...
#include <EtPipeline/RHI/Import/TextureCompression.h>
...
void EditableSdfFontAsset::SetupRuntimeAssetsInternal()
{
	...
	RefPtr<gui::SdfFontAsset> const mainAsset = Create<gui::SdfFontAsset>(*static_cast<gui::SdfFontAsset*>(m_Asset.Get()));
	ET_ASSERT(core::FileUtil::ExtractExtension(mainAsset->GetName()) == s_FontFileExt);

	m_RuntimeAssets.emplace_back(mainAsset, true);

	RefPtr<rhi::TextureAsset> const textureAsset = Create<rhi::TextureAsset>();
