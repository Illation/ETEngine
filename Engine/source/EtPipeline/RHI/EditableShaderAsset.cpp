...
{
	RefPtr<rhi::ShaderAsset> mainAsset = Create<rhi::ShaderAsset>(*static_cast<rhi::ShaderAsset*>(m_Asset.Get()));
	...
	if (m_UseFragment || m_UseGeometry)
	{
		...
		if (m_UseGeometry)
		{
			RefPtr<core::StubAsset> geoAsset = Create<core::StubAsset>();
		...
		if (m_UseFragment)
		{
			RefPtr<core::StubAsset> fragAsset = Create<core::StubAsset>();
