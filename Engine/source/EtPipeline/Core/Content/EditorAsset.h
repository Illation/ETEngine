...
protected:
	...
	{
		RuntimeAssetData(RefPtr<core::I_Asset> const asset, bool const ownsAsset);

		RefPtr<core::I_Asset> m_Asset;
		bool m_OwnsAsset = false; // #todo: with switch to refptr, is this still needed?
...
public:
	...
	void SetAsset(RefPtr<core::I_Asset> const asset) { ET_ASSERT(asset->GetType() == GetType()); m_Asset = asset; }
...
protected:
	...
	RefPtr<I_EditorAssetMeta> m_MetaData; // can stay null

	std::vector<RefPtr<EditorAssetBase>> m_ChildAssets;
...
public:
	...
	// accessors
	//-----------
	core::RawAsset<TDataType>* GetRawAsset() { return static_cast<core::RawAsset<TDataType>*>(m_Asset.Get()); }
	core::RawAsset<TDataType> const* GetRawAsset() const { return static_cast<core::RawAsset<TDataType>*>(m_Asset.Get()); }
