...
//
EditorAssetBase::RuntimeAssetData::RuntimeAssetData(RefPtr<core::I_Asset> const asset, bool const ownsAsset)
	: m_Asset(asset)
...
std::vector<EditorAssetBase::RuntimeAssetInfo> EditorAssetBase::GetAllRuntimeAssets() const
{
	...
	for (RefPtr<EditorAssetBase> const& child : m_ChildAssets)
	{
...
{
	for (RefPtr<EditorAssetBase>& child : m_ChildAssets)
	{
